...
impl TessEvaluationShader {
    pub fn id(&self) -> u32 {
        self.id
    }

    pub fn new(path: &Path) -> Result<TessEvaluationShader, String> {
        if let Ok(mut file) = File::open(path) {
                ...
                unsafe {
                    ...
                    if status == 0 {
                        ...
                        if let Ok(string) = log.into_string() {
                            Err(format!(
                                "Tessellation evaluation shader compile error:\n{}",
                                string
                            ))
                        } else {
                            Err("Tessellation evaluation shader compile error:\n<Can't convert the error log to a String>".to_string())
                        }
                    } else {
                        Ok(TessEvaluationShader { id, source })
                    }
                }
            } else {
            ...
            return Err("Tessellation evaluation shader not found!".to_string());
        }
    }
}

impl std::fmt::Debug for TessEvaluationShader {
